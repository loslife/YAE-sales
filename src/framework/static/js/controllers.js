...
angular.module('app.controllers', ['pascalprecht.translate', 'ngCookies', 'ngAnimate', 'cgBusy'])
    .controller('AppCtrl', ['$rootScope', '$scope', '$translate', '$localStorage', '$window', '$http',
        function ($rootScope, $scope, $translate, $localStorage, $window, $http) {
            $rootScope.templateUrl = "loading.html";
            $rootScope.message = '';//请稍等
            $rootScope.backdrop = false;
            $rootScope.promise = null;
            //界面刷新的时候，重新获取accesses
            /*
            $.ajax({
                url: '/svc/chain/checkAccessBySession', async: false, success: function (resp) {
                    if (resp.code != 0 || _.isEmpty(resp.result) || !_.isEmpty($rootScope.accesses)) {
                        return;
                    }
                    $rootScope.accesses = resp.result.accesses || [];
                    $rootScope.userName = resp.result.userName || "";
...
//      }
    ...
    .controller('ModalInstanceCtrl', ['$scope', '$modalInstance', 'items', function ($scope, $modalInstance, items) {
        ...
        var i=1;
        $scope.numbers = [{index: i},{index: ++i},{index: ++i},{index: ++i}];
        $scope.addStep = function(){
            $scope.numbers.push({index: ++i});
        };
